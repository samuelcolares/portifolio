...
// load events
const sectionHeaders = [document.querySelector('.section-2 h1'), document.querySelector('.section-3 h1')]
sectionHeaders.forEach(h1 => {
    h1.innerHTML = h1.textContent.split('').map((letter, idx) => `<span style="transition-delay:${idx * 60}ms">${letter}</span>`).join(``)
})
...
setTimeout(() => {
    typeText(element.nextElementSibling, 'E eu sou um front-end developer', speed)
